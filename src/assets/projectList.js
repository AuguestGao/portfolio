export const projectList = [
  {
    title: "Type-Reader",
    imageName: "type-reader home",
    purpose:
      "An online typing practice app to help users improve their typing speed and accuracy with any content they are interested in.",
    url: "/work/type_reader",
    alt: "Home page of Type Reader web app",
    buttonName: "view project",
  },
  {
    title: "Young Disciple Christian Fellowship",
    imageName: "ydcf home",
    purpose:
      "The official home to an international evangelical church (Ryerson Chapter) that aims at reaching college students.",
    url: "/work/YDCF",
    alt: "Home page of Young Disciple Christian Fellowship website",
    buttonName: "view project",
  },
  {
    title: "Work at GVA",
    imageName: "gva hero image",
    purpose:
      "Read about my past experience as a product designer and engineer at GVA, a lighting company.",
    url: "/work/GVA",
    alt: "An art was built with GVA lighting products",
    buttonName: "read story",
  },
  // {
  //   name: "Chapp",
  //   purpose:
  //     "A real-time group chat app for online communication and small file share.",
  //   url: "/work/chapp",
  //   alt: "Home page of Chapp",
  //   buttonName: "view project",
  // },
  // {
  //   name: "PM Friday",
  //   purpose:
  //     "A project management app with added personal touch to minimize the effort for time track and todos.",
  //   url: "/work/pm_friday",
  //   alt: "Home page of PM Friday with serveral cases created",
  //   buttonName: "view project",
  // },
];
